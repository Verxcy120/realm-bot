...
import { getOwnedRealms, closeRealm } from '../utils/realmsApi.js';

export const data = new SlashCommandBuilder()
    .setName('realm-close')
    .setDescription('Close your Realm to prevent players from joining')
...
export async function autocomplete(interaction) {
    ...
    try {
        if (!isUserLinked(guildId)) {
            ...
            if (ownedRealms.length === 0) {
                ...
                choices = filtered.map(realm => ({
                    name: `🏰 ${realm.name} ${realm.state === 'CLOSED' ? '(Already Closed)' : '(Open)'}`,
        ...
        console.error('[RealmClose] Autocomplete error:', error);
        try {
...
export async function execute(interaction) {
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            .setColor(0xFF0000)
            .setTitle('🔴 Realm Closed')
            .setDescription(`**${targetRealm.name}** is now closed!`)
            .addFields(
                { name: '🏰 Realm', value: targetRealm.name, inline: true },
                { name: '📊 Status', value: '🔴 Closed', inline: true }
            )
            .setFooter({ text: 'Players cannot join until you open it again' })
            .setTimestamp();
        
        return interaction.editReply({ embeds: [embed] });
        
    } catch (error) {
        console.error('[RealmClose] Error:', error);
        
        const embed = new EmbedBuilder()
            .setColor(0xFF0000)
            .setTitle('❌ Failed to Close Realm')
            .setDescription('Could not close the realm.')
            .addFields({
                name: 'Error',
